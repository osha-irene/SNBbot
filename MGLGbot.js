...
if (command === '!혼의특기') {
    ...
    if (args.length !== 1) {
        return message.reply('❌ 사용법: `!혼의특기 [특기명]` (한 단어로 입력하세요)');
    }
    ...
    if (모든특기.includes(혼의특기)) {
        return message.reply(`❌ **${혼의특기}**는 이미 존재하는 특기입니다! 혼의 특기는 기존 특기와 달라야 합니다.`);
    }
    ...
    message.reply(`**혼의 특기**가 **"${혼의특기}"**(으)로 설정되었습니다.  
💠 이 특기의 목표값은 항상 **6**입니다.`);
}
...
if (command === '!혼특확인') {
    const char = characterData[message.author.id];
    if (!char || !char.혼의특기) {
        return message.reply('❌ 설정된 혼의 특기가 없습니다.');
    }

    message.reply(`💠 **혼의 특기**: ${char.혼의특기}`);
}
// 🔹 판정 기능 (혼의 특기 & 원형 적용)
if (command === '!판정') {
    if (!characterData[message.author.id]) {
    ...
    if (args.length === 1) {
        ...
        return message.reply('❌ 사용법: `!판정 [특기]` 또는 `!판정 "원형명" [특기]`');
    }
    ...
    if (char.혼의특기 === 판정특기) {
        ...
        if (!특기좌표) {
            return message.reply(`❌ 존재하지 않는 특기입니다: **${판정특기}**`);
        }

        let 목표값 = 5;
        if (!char.특기.includes(판정특기)) {
            let 최소거리 = 99;
            for (const my특기 of char.특기) {
                for (let i = 0; i < 특기목록.length; i++) {
                    for (let j = 0; j < 특기목록[i].length; j++) {
                        if (특기목록[i][j] === my특기) {
                            let 거리 = Math.abs(i - 특기좌표.x) + Math.abs(j - 특기좌표.y) * 2;
                            if (char.영역 && (특기좌표.y === j || 특기좌표.y === j - 1 || 특기좌표.y === j + 1)) {
                                거리 = Math.abs(i - 특기좌표.x) + Math.abs(j - 특기좌표.y);
                            }
                            최소거리 = Math.min(최소거리, 거리);
                        }
                    }
                }
            }
    ...
    if (원형사용) {
        if (!char.원형 || char.원형.이름 !== 원형이름) {
            return message.reply(`❌ 당신의 원형 **"${원형이름}"**(은)는 존재하지 않습니다.`);
        }
...
// 도움 메세지
  if (message.content === '!도움') {
        const helpMessages = [
            "📜 **MGLGbot 명령어 목록 (1/6)**\n"
            + "\n**📌 캐릭터 관리**  "
            + "\n`!시트입력 \"이름\" 특기1 특기2 특기3 특기4 특기5`  "
            + "\n➥ 새로운 캐릭터를 등록하거나 기존 정보를 수정합니다.  "
            + "\n예시: `!시트입력 \"미유\" 천공 꽃 선율 사랑 잠`  "
            + "\n`!지정 \"캐릭터 이름\"`  "
            + "\n➥ 특정 캐릭터를 활성화합니다.  "
            + "\n`!지정해제`  "
            + "\n➥ 현재 활성화된 캐릭터를 해제합니다.  "
            ...
            + "\n➥ 능력치를 조정합니다. (1~7 범위)  "
            + "\n`!계제 3`  "
            + "\n➥ 캐릭터의 계제를 설정합니다. (기본값: 3)  "
            + "\n`!기관 [원탁/천애/학원/문호/엽귀/아방궁]`  "
            + "\n➥ 캐릭터의 기관을 설정합니다.  "
            + "\n`!위계 [서경/사서/서공/방문자/이단자/외전]`  "
            + "\n➥ 캐릭터의 위계를 설정합니다.  ",

            "📜 **MGLGbot 명령어 목록 (3/6)**\n"
            + "\n**📌 혼의 특기**  "
            + "\n`!혼의특기 [특기명]`  "
            + "\n➥ 기존 특기와 겹치지 않는 새로운 특기를 설정합니다.  "
            + "\n예시: `!혼의특기 창조`  "
            + "\n🔹 **혼의 특기란?**"
            + "\n혼의 특기는 마법사의 정체성을 나타내는 고유 특기입니다. "
            + "\n이 특기를 사용한 판정은 **목표값 6**으로 고정됩니다.  "
            + "\n`!혼특확인`  "
            + "\n➥ 설정된 혼의 특기를 확인합니다.  ",
            ...
            + "\n(혼의 특기 자동 적용)  "
            + "\n`!판정 \"원형이름\" [특기]`  "
            + "\n➥ 특정 원형을 이용하여 판정을 수행합니다.  "
            + "\n예시: `!판정 \"마왕\" 분노`  "
            + "\n`!장서 \"장서명\"`  "
            + "\n➥ 보유한 장서를 사용하여 판정을 수행합니다.  ",
            ...
            + "\n➥ 현재 저장된 모든 플롯을 공개합니다.  "
            + "\n\n**📌 장서 관리**  "
            + "\n`!장서추가 \"장서명\" 타입 특기 코스트영역 코스트개수 설명`  "
            + "\n➥ 장서를 추가합니다.  "
            + "\n예시: `!장서추가 \"마검 소환\" 공격 선율 노래 2 강력한 검을 소환한다.`  "
            + "\n`!장서삭제 \"장서명\"`  "
            + "\n➥ 특정 장서를 삭제합니다.  "
            + "\n`!장서목록`  "
            + "\n➥ 보유한 장서를 확인합니다.  "
            + "\n`!장서리셋`  "
            + "\n➥ 모든 장서를 삭제합니다. (`\"긴급 소환\"` 유지)  "
            + "\n\n📌 추가적인 도움은 오샤님께 문의하세요."
        ];
