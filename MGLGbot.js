...
🛠️ 기본 명령어
\`\`\`
**\`!도움\`** - 사용 가능한 모든 명령어를 확인합니다.  
**\`!시트확인\`** - 현재 설정된 캐릭터 정보를 확인합니다.  

\`\`\`yaml
🧑‍🎓 캐릭터 관리
\`\`\`
**\`!시트입력 [이름] [특기1] [특기2] [특기3] [특기4] [특기5]\`**  
캐릭터를 등록합니다. 특기 5개를 반드시 입력해야 합니다.  
예시: \`!시트입력 이솔렛 천공 꽃 선율 사랑 잠\`  

**\`!시트삭제\`** - 현재 캐릭터 정보를 삭제합니다.  
**\`!지정\`** - 서버별로 사용할 캐릭터를 설정합니다.  
**\`!지정해제\`** - 현재 서버에서 캐릭터를 해제합니다.  
**\`!기관 [원탁/천애/학원/문호/엽귀/아방궁]\`** - 캐릭터의 기관을 설정합니다.  
**\`!계제 [1~7]\`** - 캐릭터의 계제를 설정합니다. (기본값: 3)  
**\`!위계 [서경/사서/서공/방문자/이단자/외전]\`** - 캐릭터의 위계를 설정합니다.  
`
    );
...
📖 특기 및 영역 관리
\`\`\`
**\`!특기설정 [특기1] [특기2] [특기3] [특기4] [특기5]\`**  
특기를 변경할 수 있습니다. (총 5개)  
예시: \`!특기설정 천공 꽃 선율 사랑 잠\`  

**\`!특기확인\`** - 현재 설정된 특기를 확인합니다.  
**\`!영역 [별/짐승/힘/노래/꿈/어둠]\`** - 캐릭터의 영역을 설정합니다.  

\`\`\`yaml
🎲 판정 시스템
\`\`\`
**\`!판정 [특기]\`** - 특정 특기로 판정을 수행합니다.  
예시: \`!판정 천공\`  

**\`!장서 [장서명]\`** - 보유한 장서를 사용하여 판정을 수행합니다.  
예시: \`!장서 "마검 소환"\`  

\`\`\`yaml
📜 장서 관리
\`\`\`
**\`!장서추가 "장서명" [판정특기] [마소영역][코스트] [설명]\`**  
새로운 장서를 추가합니다.  
예시: \`!장서추가 "마검 소환" 선율 노래2 강력한 검을 소환한다.\`  

**\`!장서삭제 "장서명"\`** - 특정 장서를 삭제합니다.  
**\`!장서리셋\`** - 모든 장서를 삭제합니다. (되돌릴 수 없음)  
**\`!장서목록\`** - 현재 보유한 장서 목록을 확인합니다.  
`
    );
}
});
